...
contract Farm is AccessControlEnumerable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    // The Protocol Token!
    LystToken public lyst;
    // Dev address.
    address public devaddr;
    // lyst tokens created per block.
    uint256 public lystPerBlock;
    // Deposit Fee address
    ...
    constructor(
        ...
        address _timelock,
        address _feeAddress,
        uint256 _lystPerBlock,
        uint256 _startBlock
    ) public {
        lyst = _lyst;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        lystPerBlock = _lystPerBlock;
    ...
    struct UserInfo {
        ...
        //      pending reward = (user.amount * pool.acclystPerShare) - user.rewardDebt

        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `acclystPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accLYSTPerShare; // Accumulated LYST's per share, times 1e12. See below.
    ...
    // The block number when LYST mining starts.
    ...
    // View pending LYSTs rewards.
    function pendingLYST(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 lystReward =
                blockDifference.mul(lystPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accLYSTPerShare = accLYSTPerShare.add(
                lystReward.mul(1e12).div(lpSupply)
            );
        }
        return user.amount.mul(accLYSTPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // Deposit pool tokens for LYST allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            uint256 pending =
                user.amount.mul(pool.accLYSTPerShare).div(1e12).sub(
                    user.rewardDebt
                );
            // When user executes deposit, pending rewards get sent to the user.
            if (pending > 0) {
                safeLYSTTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accLYSTPerShare).div(1e12);
    ...
    // In case if rounding error causes pool to not have enough LYST TOKENS
    function safeLYSTTransfer(address _to, uint256 _amount) internal {
        // Check how many LYST token's on the protocol address.
        uint256 LYSTBal = lyst.balanceOf(address(this));
    ...
    // Fees will be used to buy back LYST tokens.
    function setFeeAddress(address _feeAddress) public {
        require(msg.sender == feeAddress, "setFeeAddress: FORBIDDEN");
        feeAddress = _feeAddress;
    }

    // Function that sets the new amount of how many new LYST Tokens will be minted per each block.
    // TEMP DISABLE ONLY OWNER.
    function updateEmissionRate(uint256 _LYSTPerBlock) public {
        massUpdatePools();
        lystPerBlock = _LYSTPerBlock;
    }
}
